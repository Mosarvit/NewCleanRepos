 ...
 */
bool Graph::findShortestPathDijkstra(std::deque<Edge*>& rPath, Node* pSrcNode, Node* pDstNode, int preisstufe)
{
	...
	while (!Q.empty())
	{
		...
		for (std::list<Edge*>::iterator it = rOutEdges.begin(); it != rOutEdges.end(); it++)
		{
			...
			double newDistance = rCurrNodeEntry.dist + dynamic_cast<PreisAngebot*>(*it)->getWeight(preisstufe);
